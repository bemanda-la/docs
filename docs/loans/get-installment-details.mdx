...
`GET /v1/loans/installment/:loan_id`
...
```bash
curl -X GET \
  {base_url}/v1/loans/installment/{loan_id}?page=1&items_per_page=10 \
...
async function getInstallmentsByLoanId(
  ...
  const response = await fetch(
    `${base_url}/v1/loans/installment/${loan_id}?page=${page}&items_per_page=${itemsPerPage}`,
    {
      method: "GET",
      headers: {
