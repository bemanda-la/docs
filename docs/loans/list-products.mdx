...
- `status` (opcional): Filtrar productos por estado: `Available`.
- `orderByField` (opcional): Campo por el cual se desea ordenar los productos. Valores permitidos: `id`, `status`, `granted_capital`, `inactivation_date`.
- `orderByDirection` (opcional): Dirección del ordenamiento. Valor por defecto: `desc`: Orden descendente. `asc`: Orden ascendente.
...
async function getProducts(page = 1, itemsPerPage = 50, token, status, orderByField, orderByDirection) {
  ...
  const response = await fetch(`${base_url}/v1/loans/apps/products?${queryParams}`, {
    method: "GET",
    headers: {
...
getProducts(page, itemsPerPage, token, "Available", "status", "asc").catch(console.error);
...
- **status:** Información del nombre del estado actual del producto. (Available).
- **name:** Informacion del nombre del producto.
- **granted_capital:** Capital otorgado total de los préstamos asociados a este producto.
