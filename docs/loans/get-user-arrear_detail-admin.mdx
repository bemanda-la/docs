# Consulta de Detalles de Mora de un Usuario (nivel administrador)
...
- **user_id** (opcional): Identificador del usuario asociado al atraso.
...
```bash
curl -X GET \
  {base_url}/v1/apps/arrears_detail/?{loan_id}&page=1&items_per_page=10 \
...
async function getArrearsDetail(
  ...
  const response = await fetch(
    `${base_url}/v1/apps/arrears_detail/?loan_id=${loanId}&start_date=${startDate}&end_date=${endDate}&page=${page}&items_per_page=${itemsPerPage}`,
    {
      method: "GET",
      headers: {
